...
SRC = ROOT / "example" / "gemini_ai_example.json"
EXP_CHATS = 2
EXP_MSGS = 4
STATS = "2 chats &middot; 4 messages"
DOM_NEEDLES = [
    "Режим ИИ: Выполнен поиск Example question about git branch workflow.",
    "Режим ИИ: Выполнен поиск Example second query about cloning without the .git folder.",
    "Example answer text that explains how to set an upstream branch in general terms.",
]
INDEX_NEEDLES = [
    '"t": "example question about git branch workflow."',
    '"t": "example answer text that explains how to set an upstream branch in general terms."',
    '"fmt": "gemini_activity"',
]
STDOUT_NEEDLES = ["[gemini_activity]", "2 chat(s), 4 msg(s)"]


class TestGeminiAiE2E(unittest.TestCase):
    def test_generate_archive(self):
        tmp = tempfile.mkdtemp(prefix="aia_e2e_gemini_ai_")
