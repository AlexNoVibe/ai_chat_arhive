...
SRC = ROOT / "example" / "gemini_apps_example.json"
EXP_CHATS = 2
EXP_MSGS = 4
STATS = "2 chats &middot; 4 messages"
DOM_NEEDLES = [
    "Приложения Gemini: Отправлен запрос Example question about a coding project setup.",
    "Приложения Gemini: Отправлен запрос Example second question about a generic placeholder.",
    "Example answer text that explains the requested topic in general terms.",
]
INDEX_NEEDLES = [
    "example question about a coding project setup.",
    "example answer text that explains the requested topic in general terms.",
    '"fmt": "gemini_activity"',
]
...
class TestGeminiAppsE2E(unittest.TestCase):
    def test_generate_archive(self):
        tmp = tempfile.mkdtemp(prefix="aia_e2e_gemini_apps_")
