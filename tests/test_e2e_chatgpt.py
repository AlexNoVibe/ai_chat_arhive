...
SRC = ROOT / "example" / "chatgpt_example.json"
EXP_CHATS = 2
EXP_MSGS = 8
STATS = "2 chats &middot; 8 messages"
DOM_NEEDLES = ["Example Title Placeholder", "text-davinci-002-render"]
INDEX_NEEDLES = [
    "example text here. example text here. example text here.",
    "example title example text example title example text.",
    '"fmt": "chatgpt"',
]
STDOUT_NEEDLES = ["[chatgpt]", "2 chat(s), 8 msg(s)"]


class TestChatgptE2E(unittest.TestCase):
    def test_generate_archive(self):
        tmp = tempfile.mkdtemp(prefix="aia_e2e_chatgpt_")
        ...
        self.assertEqual(html.count('class="message '), EXP_MSGS, "message div count")
